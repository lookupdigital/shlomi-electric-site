...
const docJson = z
  ...
  .transform((value, ctx) => {
    try {
      ...
      if (doc?.type === "doc" && Array.isArray(doc.content)) return doc as Record<string, unknown>;
    } catch {
      // fall through
    }
    ctx.addIssue({ code: "custom", message: "תוכן לא תקין" });
    return z.NEVER;
  });

const postSchema = z.object({
  id: optional(z.uuid()),
  title: z.string().trim().min(1, "נא למלא כותרת").max(200),
...
const LABELS = {
  title: "כותרת",
  slug: "Slug",
  featured_image_url: "תמונה ראשית",
...
export async function savePost(formData: FormData): Promise<FormState> {
  ...
  if (!SLUG_PATTERN.test(slug)) return { ok: false, message: "Slug: השתמשו באותיות, מספרים ומקפים" };
  ...
  if (id) {
    ...
    if (!data?.length) return { ok: false, message: "הפוסט לא נמצא או שאין הרשאה." };
  } else {
  ...
  return { ok: true, message: record.status === "published" ? "הפוסט נשמר ומפורסם." : "הטיוטה נשמרה." };
}

function postError(error: { code?: string; message: string }) {
  if (error.code === "23505") return "קיים כבר פוסט עם ה-slug הזה.";
